...
|---|---|---|
| A, cross-domain | TableShift, 7 domain-split tasks (health x2, income, public policy, labour, education x2) | ID coverage within 0.01 of 0.95 on all 7; OOD coverage falls on 6 of 7 (by 0.6 to 36 points); REVIEW rate rises under shift on 6 of 7; ALARM on 100% of OOD streams on 5 tasks, 60% ACS income, 40% ASSISTments; 0 to 4% false alarms on 6 tasks and 27% on hospital readmission, diagnosed (sparse one-hot inputs, 4,286 calibration rows, per-column scaling); ASSISTments (school shift): coverage 0.95 to 0.59, REVIEW rate falls, input tripwire misses it, outcome verdict catches it at 44x the claim (`TABLESHIFT.md`, `martingale_diag.json`) |
